...
class Board:
    ...
    def __init__(self, board_w, board_h, num_players, piece_list, starting_point=(0, 0)):
        ...
        self.connected = np.full(
            (num_players, board_h, board_w), False, np.bool_)
        self.connected[0, starting_point[0], starting_point[1]] = True
        self.piece_list = piece_list
        self.pieces = np.full(
            (num_players, piece_list.get_num_pieces()), True, np.bool_)
    ...
    def __copy__(self):
        cpy_board = Board(self.board_w, self.board_h,
                          self.num_players, self.piece_list)
